...
MinotaurOS 目前可以运行在 QEMU 虚拟机和 Vision Five 2 硬件平台，通过解析设备树实现一套二进制兼容不同硬件。MinotaurOS 支持了 70 个 Linux 系统调用，并且能够通过一系列测试用例，如#[@fig:pre-2024]。
...
#h(2em) 与此同时，MinotaurOS 还能够支持 busybox、lua 等 Linux 程序的运行。#[@fig:lua_test]展示了 MinotaurOS 运行 lua 脚本的效果。

#figure(
  image("img/lua_test.png", width: 90%),
  caption: [运行 lua],
  supplement: [图],
)<lua_test>
