...
pub async fn sys_futex(
    ...
    let cval = proc_inner
        .addr_space
        .user_slice_r(VirtAddr(uaddr), size_of::<u32>())?;
    let cval = unsafe {
        AtomicU32::from_ptr(cval.as_ptr().cast::<u32>().cast_mut()).load(Ordering::Relaxed)
    };
    ...
    match op {
        FutexOp::Wait => {
            if cval == val {
                ...
                let future = current_thread()
                    .event_bus
                    .suspend_with(Event::all(), FutexFuture::new(VirtAddr(uaddr), val));
                match timeout {
        ...
        FutexOp::Requeue | FutexOp::CmpRequeue => {
            ...
            proc_inner
                .addr_space
                .user_slice_r(VirtAddr(uaddr2), size_of::<u32>())?;
            let cnt = proc_inner.futex_queue.requeue(
                VirtAddr(uaddr),
                VirtAddr(uaddr2),
                val as usize,
                val2,
